...
export function ExecutionTimeline({
  ...
  return (
    <div className="space-y-6 w-full">
      <div className="p-4 rounded-2xl border border-zinc-200 bg-white shadow-sm space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2 text-sm text-zinc-700">
            <span className="px-2.5 py-1 rounded-full bg-zinc-900 text-white text-[11px] font-semibold">
              {isClient ? "Client view" : "Team view"}
            </span>
          ...
          <div className="flex flex-wrap items-center gap-2 text-[12px] font-semibold">
            ...
            {disputeCount > 0 && (
              <span className="px-2.5 py-1 rounded-full bg-red-50 text-red-700 border border-red-100">
      ...
      <div className="relative space-y-8 pt-2">
        <div className="absolute left-[27px] sm:left-[35px] top-2 bottom-4 w-[2px] bg-zinc-100 rounded-full" />
        <motion.div
          initial={{ height: 0 }}
          animate={{ height: `${progressPercent}%` }}
          transition={{ duration: 1.2, ease: "circOut" }}
          className="absolute left-[27px] sm:left-[35px] top-2 w-[2px] bg-zinc-900 rounded-full z-0"
